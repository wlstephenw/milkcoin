...
contract('MasterChef', ([alice, bob, carol, dev, minter]) => {
    ...
    context('With ERC/LP token added to the field', () => {
        ...
        it('should allow emergency withdraw', async () => {
            ...
            await this.chef.deposit(0, 0, '100', { from: bob });
        ...
        it('should give out SUSHIs only after farming time', async () => {
            ...
            await this.chef.deposit(0, 0, '0', { from: bob }); // block 90
            assert.equal((await this.milk.balanceOf(bob)).valueOf(), '0');
            await time.advanceBlockTo('94');
            await this.chef.deposit(0, 0, '0', { from: bob }); // block 95
            assert.equal((await this.milk.balanceOf(bob)).valueOf(), '0');
            await time.advanceBlockTo('99');
            await this.chef.deposit(0, 0, '0', { from: bob }); // block 100
            assert.equal((await this.milk.balanceOf(bob)).valueOf(), '0');
            await time.advanceBlockTo('100');
            await this.chef.deposit(0, 0, '0', { from: bob }); // block 101
            assert.equal((await this.milk.balanceOf(bob)).valueOf(), '1000');
            await time.advanceBlockTo('104');
            await this.chef.deposit(0, 0, '0', { from: bob }); // block 105
            assert.equal((await this.milk.balanceOf(bob)).valueOf(), '5000');
            assert.equal((await this.milk.totalSupply()).valueOf(), '5000');
        });

        it('should not distribute SUSHIs if no one deposit', async () => {
            // 100 per block farming rate starting at block 200 with bonus until block 1000
            this.chef = await MasterChef.new(this.milk.address, '100', '200', '1000', { from: alice });
            await this.milk.transferOwnership(this.chef.address, { from: alice });
            await this.chef.add('100', [this.lp.address, this.lp2.address], true);
            await this.lp.approve(this.chef.address, '1000', { from: bob });
            await time.advanceBlockTo('199');
            assert.equal((await this.milk.totalSupply()).valueOf(), '0');
            await time.advanceBlockTo('204');
            assert.equal((await this.milk.totalSupply()).valueOf(), '0');
            await time.advanceBlockTo('209');
            await this.chef.deposit(0, 0, '10', { from: bob }); // block 210
            assert.equal((await this.milk.totalSupply()).valueOf(), '0');
            assert.equal((await this.milk.balanceOf(bob)).valueOf(), '0');
            assert.equal((await this.lp.balanceOf(bob)).valueOf(), '990');
            await time.advanceBlockTo('219');
            await this.chef.withdraw(0, 0, '10', { from: bob }); // block 220
            assert.equal((await this.milk.totalSupply()).valueOf(), '10000');
            assert.equal((await this.milk.balanceOf(bob)).valueOf(), '10000');
            assert.equal((await this.lp.balanceOf(bob)).valueOf(), '1000');
        });
        ...
        it('should distribute SUSHIs properly for each staker', async () => {
            ...
            await this.chef.deposit(0, 0, '20', { from: bob });
            // Carol deposits 30 LPs at block 318
            await time.advanceBlockTo('317');
            await this.chef.deposit(0, 0, '30', { from: carol });
            ...
            await this.chef.withdraw(0, 0, '5', { from: bob });
            ...
            await this.chef.withdraw(0, 0, '15', { from: bob });
            await time.advanceBlockTo('359')
            await this.chef.withdraw(0, 0, '30', { from: carol });
        ...
        it('should give proper SUSHIs allocation to each pool', async () => {
            ...
            this.chef = await MasterChef.new(this.milk.address, '100', '400', '1000', { from: alice });
            await this.milk.transferOwnership(this.chef.address, { from: alice });
            await this.lp.approve(this.chef.address, '1000', { from: alice });
            ...
            assert.equal((await this.chef.pendingMilk(0, alice)).valueOf(), '10000');
            // Bob deposits 10 LP2s at block 425
            await time.advanceBlockTo('424');
            await this.chef.deposit(1, 0, '5', { from: bob });
            // Alice should have 10000 + 5*1/3*1000 = 11666 pending reward
            assert.equal((await this.chef.pendingMilk(0, alice)).valueOf(), '11666');
            await time.advanceBlockTo('430');
            // At block 430. Bob should get 5*2/3*1000 = 3333. Alice should get ~1666 more.
            assert.equal((await this.chef.pendingMilk(0, alice)).valueOf(), '13333');
            assert.equal((await this.chef.pendingMilk(1, bob)).valueOf(), '3333');
        });

        it('should stop giving bonus SUSHIs after the bonus period ends', async () => {
            // 100 per block farming rate starting at block 500 with bonus until block 600
            this.chef = await MasterChef.new(this.milk.address, '100', '500', '600', { from: alice });
            await this.milk.transferOwnership(this.chef.address, { from: alice });
            await this.lp.approve(this.chef.address, '1000', { from: alice });
            await this.chef.add('1', [this.lp.address, this.lp2.address], true);
            // Alice deposits 10 LPs at block 590
            await time.advanceBlockTo('589');
            await this.chef.deposit(0, 0, '10', { from: alice });
            // At block 605, she should have 1000*10 + 100*5 = 10500 pending.
            await time.advanceBlockTo('605');
            assert.equal((await this.chef.pendingMilk(0, alice)).valueOf(), '10500');
            // At block 606, Alice withdraws all pending rewards and should get 10600.
            await this.chef.deposit(0, 0, '0', { from: alice });
            assert.equal((await this.chef.pendingMilk(0, alice)).valueOf(), '0');
            assert.equal((await this.milk.balanceOf(alice)).valueOf(), '10600');
        });
    });
});
